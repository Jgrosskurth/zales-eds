/* =============================================================================
   ZALES EDS — footer.js
   Multi-column footer with social icons, link groups, email signup, legal bar.
   ============================================================================= */
...
const SOCIAL_LINKS = [
  {
    ...
    icon: `<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.5" cy="6.5" r="1" fill="currentColor"/></svg>`,
  },
  {
    name: 'Facebook',
    href: 'https://facebook.com/zales',
    icon: `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/></svg>`,
  },
  {
    name: 'Pinterest',
    href: 'https://pinterest.com/zalesjewelry',
    icon: `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 2C6.48 2 2 6.48 2 12c0 4.24 2.64 7.88 6.38 9.35-.09-.81-.17-2.06.03-2.94.19-.8 1.27-5.38 1.27-5.38s-.32-.65-.32-1.6c0-1.5.87-2.62 1.95-2.62.92 0 1.37.69 1.37 1.52 0 .93-.59 2.31-.9 3.6-.25 1.07.54 1.95 1.59 1.95 1.91 0 3.19-2.43 3.19-5.3 0-2.19-1.49-3.73-3.62-3.73-2.47 0-3.92 1.85-3.92 3.77 0 .74.29 1.55.64 1.99.07.09.08.17.06.26-.07.27-.22.85-.25.97-.04.16-.13.19-.3.12-1.12-.52-1.82-2.16-1.82-3.48 0-2.83 2.06-5.43 5.93-5.43 3.11 0 5.53 2.22 5.53 5.18 0 3.09-1.95 5.58-4.66 5.58-.91 0-1.77-.47-2.06-1.03l-.56 2.1c-.2.78-.75 1.75-1.12 2.34.84.26 1.74.4 2.67.4 5.52 0 10-4.48 10-10S17.52 2 12 2z"/></svg>`,
  },
  {
    name: 'TikTok',
    href: 'https://tiktok.com/@zales',
    icon: `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-2.88 2.5 2.89 2.89 0 0 1-2.89-2.89 2.89 2.89 0 0 1 2.89-2.89c.28 0 .54.04.79.1V9.01a6.33 6.33 0 0 0-.79-.05 6.33 6.33 0 0 0-6.34 6.33 6.33 6.33 0 0 0 6.34 6.33 6.33 6.33 0 0 0 6.33-6.33V8.88a8.19 8.19 0 0 0 4.79 1.52V7.01a4.85 4.85 0 0 1-1.02-.32z"/></svg>`,
  },
  {
    name: 'YouTube',
    href: 'https://youtube.com/zales',
    icon: `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M22.54 6.42a2.78 2.78 0 0 0-1.95-1.96C18.88 4 12 4 12 4s-6.88 0-8.59.46a2.78 2.78 0 0 0-1.95 1.96A29 29 0 0 0 1 12a29 29 0 0 0 .46 5.58A2.78 2.78 0 0 0 3.41 19.6C5.12 20 12 20 12 20s6.88 0 8.59-.4a2.78 2.78 0 0 0 1.95-1.95A29 29 0 0 0 23 12a29 29 0 0 0-.46-5.58z"/><polygon points="9.75 15.02 15.5 12 9.75 8.98 9.75 15.02" style="fill:var(--color-footer-bg,#111);"/></svg>`,
  },
];
...
function buildSocialIcons() {
  return SOCIAL_LINKS.map((s) => `
    <a href="${s.href}" class="footer-social-link" aria-label="${s.name}" target="_blank" rel="noopener noreferrer">
      ${s.icon}
    </a>`).join('');
}

/**
 * Build a footer link column with mobile accordion.
 */
function buildLinkGroup(group, index) {
  const linksHTML = group.links.map((l) => `<li><a href="${l.href}">${l.label}</a></li>`).join('');
...
/**
 * Build payment icons row.
 */
function buildPaymentIcons() {
  return PAYMENT_METHODS.map((m) => `<span class="footer-payment-icon">${m}</span>`).join('');
}
  ...
  footerEl.innerHTML = `
    <div class="footer-main">
      ...
      <div class="footer-brand">
        ...
        <p class="footer-tagline">America's Diamond Store Since 1924. Fine jewelry for every love story, every look, every life.</p>
        <div class="footer-social" role="list" aria-label="Follow us on social media">
          ${buildSocialIcons()}
        </div>
        <div class="footer-signup">
          <span class="footer-signup-label">Stay in the Know</span>
          <form class="footer-signup-form" aria-label="Email signup" onsubmit="return false;">
            <input
              type="email"
    ...
    <div class="footer-bottom">
      <p class="footer-copyright">
        © ${year} Zales Corporation. All Rights Reserved. Signet Jewelers.
      </p>
