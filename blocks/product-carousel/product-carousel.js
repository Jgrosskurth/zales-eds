/* =============================================================================
   ZALES EDS — product-carousel.js
   Horizontal snap-scrolling carousel with arrow navigation and wishlist.

   Block table structure:
     Row 0: heading | "View All" link
     Row 1+: product image | product name | price | sale price (optional) | product href
   ============================================================================= */
...
/**
 * Parse products from block rows.
 */
function parseProducts(rows) {
  ...
  return rows.map((row) => {
    ...
    const linkEl = cells[4]?.querySelector('a') || cells[3]?.querySelector('a');
    return {
      imgSrc: imgEl?.src || null,
      imgAlt: imgEl?.alt || '',
      name: cells[1]?.textContent.trim() || '',
      price: cells[2]?.textContent.trim() || '',
      salePrice: cells[3]?.textContent.trim() || null,
      href: linkEl?.href || '#',
    };
  }).filter((p) => p.name);
}
...
function buildProductCard(product, index) {
  ...
  const imgHTML = product.imgSrc
    ? `<img class="product-card-image" src="${product.imgSrc}" alt="${product.imgAlt}" loading="${index < 4 ? 'eager' : 'lazy'}" decoding="async"/>`
    : `<div class="product-card-placeholder">${product.name}</div>`;

  card.innerHTML = `
    <a href="${product.href}" class="product-card-link">
      <div class="product-card-image-wrapper">
        ${imgHTML}
        <button class="product-card-wishlist" aria-label="Add ${product.name} to wishlist" data-product-id="${index}">
          ${HEART_ICON}
        </button>
      </div>
  ...
  // Wishlist toggle (stop propagation so link doesn't fire)
...
export default function decorate(block) {
  ...
  if (rows.length > 0) {
    ...
    const possibleHeading = headerCells[0]?.textContent.trim();
    const possibleViewAll = headerCells[1]?.querySelector('a');

    // Only treat row 0 as header if it has no image
    if (!rows[0].querySelector('img') && possibleHeading) {
      heading = possibleHeading;
      if (possibleViewAll) viewAllHref = possibleViewAll.href;
  ...
  block.innerHTML = `
    ...
    <div class="product-carousel-track-wrapper">
      <button class="carousel-arrow prev" aria-label="Previous products">
        ${ARROW_LEFT}
      </button>
  ...
  products.forEach((product, i) => {
    track.appendChild(buildProductCard(product, i));
  });
